...
/**
 * A thread-safe library to measure energy consumption using Intel Running
 * Average Power Limit (RAPL) sysfs files. The library handles overflows in the
 * RAPL files using an extended energy counter for each file that is
 * periodically updated by a thread.
 *
...
/**
 * Destroy the RAPL-stopwatch API. Clean all dynamic memory allocated when
 ...
 * @param total_mj_domain On return, it will contain the total energy
 * consumption in mJ.
 * @return a rapl_error_t.
 */
